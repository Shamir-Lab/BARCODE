 ...
 * trie.
 */
value_t* hattrie_get (hattrie_t*, const char* key, long long len);


/** Find a given key in the table, returning a NULL pointer if it does not
 * exist. */
value_t* hattrie_tryget (hattrie_t*, const char* key, long long len);
...
const char*     hattrie_iter_key       (hattrie_iter_t*, long long* len);
