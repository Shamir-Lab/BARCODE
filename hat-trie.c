...
static const long long MAX_BUCKET_SIZE = 16384;
...
struct hattrie_t_
{
    ...
    long long m;      // number of stored keys
};
...
static node_ptr hattrie_consume(node_ptr *p, const char **k, long long *l, unsigned brk)
{
...
/* find node in trie */
static node_ptr hattrie_find(hattrie_t* T, const char **key, long long *len)
{
...
static void hattrie_split(hattrie_t* T, node_ptr parent, node_ptr node)
{
    ...
    for (num_slots = ahtable_initial_size;
            (long long) left_m > ahtable_max_load_factor * (long long) num_slots;
            num_slots *= 2);
    ...
    for (num_slots = ahtable_initial_size;
            (long long) right_m > ahtable_max_load_factor * (long long) num_slots;
            num_slots *= 2);
...
value_t* hattrie_get(hattrie_t* T, const char* key, long long len)
{
    node_ptr parent = T->root;
    assert(*parent.flag & NODE_TYPE_TRIE);
...
value_t* hattrie_tryget(hattrie_t* T, const char* key, long long len)
{
...
int hattrie_del(hattrie_t* T, const char* key, long long len)
{
    node_ptr parent = T->root;
    assert(*parent.flag & NODE_TYPE_TRIE);
    ...
    /* remove from bucket */
    long long m_old = ahtable_size(node.b);
...
{
    char* key;
    long long keysize; // space reserved for the key
    long long level;
...
static void hattrie_iter_pushchar(hattrie_iter_t* i, long long level, char c)
{
    if (i->keysize < level) {
        i->keysize *= 2;
...
const char* hattrie_iter_key(hattrie_iter_t* i, long long* len)
{
