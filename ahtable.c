...
const long long ahtable_max_load_factor = 100000.0; /* arbitrary large number => don't resize */
const long long ahtable_initial_size = 4096;

static long long keylen(slot_t s) {
    if (0x1 & *s) {
        return (long long) (*((uint16_t*) s) >> 1);
    }
    else {
...
ahtable_t* ahtable_create_n(long long n)
{
    ...
    table->n = n;
    table->m = 0;
    table->max_m = (long long) (ahtable_max_load_factor * (long long) table->n);
    table->slots = malloc_or_die(n * sizeof(slot_t));
    memset(table->slots, 0, n * sizeof(slot_t));

    table->slot_sizes = malloc_or_die(n * sizeof(long long));
    memset(table->slot_sizes, 0, n * sizeof(long long));
...
long long ahtable_size(const ahtable_t* table)
{
    return table->m;
}
...
void ahtable_clear(ahtable_t* table)
{
    ...
    table->slot_sizes = realloc_or_die(table->slot_sizes, table->n * sizeof(long long));
    memset(table->slot_sizes, 0, table->n * sizeof(long long));
}

/** Inserts a key with value into slot s, and returns a pointer to the
  * space immediately after.
  */
static slot_t ins_key(slot_t s, const char* key, long long len, value_t** val)
{
    // key length
    if (len < 128) {
...
static void ahtable_expand(ahtable_t* table)
{
     ...
     */
    assert(table->n > 0);
    long long new_n = 2 * table->n;
    long long* slot_sizes = malloc_or_die(new_n * sizeof(long long));
    memset(slot_sizes, 0, new_n * sizeof(long long));
...
static value_t* get_key(ahtable_t* table, const char* key, long long len, bool insert_missing)
{
    ...
    long long i = hash(key, len) % table->n;
    long long k;
    slot_t s;
    value_t* val;
    ...
    s = table->slots[i];
    while ((long long) (s - table->slots[i]) < table->slot_sizes[i]) {
        /* get the key length */
        k = keylen(s);
        s += k < 128 ? 1 : 2;
    ...
    if (insert_missing) {
        ...
        long long new_size = table->slot_sizes[i];
...
value_t* ahtable_get(ahtable_t* table, const char* key, long long len)
{
    return get_key(table, key, len, true);
}


value_t* ahtable_tryget(ahtable_t* table, const char* key, long long len )
{
    return get_key(table, key, len, false);
}


int ahtable_del(ahtable_t* table, const char* key, long long len)
{
    long long i = hash(key, len) % table->n;
    long long k;
    slot_t s;
    ...
    while ((long long) (s - table->slots[i]) < table->slot_sizes[i]) {
        ...
        if (memcmp(s, key, len) == 0) {
            ...
            s -= k < 128 ? 1 : 2;
            memmove(s, t, table->slot_sizes[i] - (long long) (t - table->slots[i]));
            table->slot_sizes[i] -= (long long) (t - s);
            --table->m;
            return 0;
        }
        /* key not found. */
...
static int cmpkey(const void* a_, const void* b_)
{
    ...
    long long ka = keylen(a), kb = keylen(b);
...
static const char* ahtable_sorted_iter_key(ahtable_sorted_iter_t* i, long long* len)
{
...
static ahtable_unsorted_iter_t* ahtable_unsorted_iter_begin(const ahtable_t* table)
{
    ...
    for (i->i = 0; i->i < i->table->n; ++i->i) {
        i->s = table->slots[i->i];
        if ((long long) (i->s - table->slots[i->i]) >= table->slot_sizes[i->i]) continue;
        break;
    }
...
static void ahtable_unsorted_iter_next(ahtable_unsorted_iter_t* i)
{
    ...
    /* get the key length */
    long long k = keylen(i->s);
    i->s += k < 128 ? 1 : 2;

    /* skip to the next key */
    i->s += k + sizeof(value_t);

    if ((long long) (i->s - i->table->slots[i->i]) >= i->table->slot_sizes[i->i]) {
        do {
            ++i->i;
        } while(i->i < i->table->n &&
...
static const char* ahtable_unsorted_iter_key(ahtable_unsorted_iter_t* i, long long* len)
{
...
const char* ahtable_iter_key(ahtable_iter_t* i, long long* len)
{
