...
typedef struct ahtable_t_
{
    ...
    long long n;        // number of slots
    long long m;        // number of key/value pairs stored
    long long max_m;    // number of stored keys before we resize

    long long*  slot_sizes;
    slot_t*  slots;
} ahtable_t;

extern const long long ahtable_max_load_factor;
extern const long long ahtable_initial_size;

ahtable_t* ahtable_create   (void);         // Create an empty hash table.
ahtable_t* ahtable_create_n (long long n);     // Create an empty hash table, with
                                            //  n slots reserved.

void       ahtable_free   (ahtable_t*);       // Free all memory used by a table.
void       ahtable_clear  (ahtable_t*);       // Remove all entries.
long long     ahtable_size   (const ahtable_t*); // Number of stored keys.
 ...
 * table.
 */
value_t* ahtable_get (ahtable_t*, const char* key, long long len);


/* Find a given key in the table, return a NULL pointer if it does not exist. */
value_t* ahtable_tryget (ahtable_t*, const char* key, long long len);


int ahtable_del(ahtable_t*, const char* key, long long len);
...
const char*     ahtable_iter_key       (ahtable_iter_t*, long long* len);
